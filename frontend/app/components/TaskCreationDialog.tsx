...
export default function TaskCreationDialog({
  ...
  return (
    <div
      aria-label="Task Creation Dialog"
      className={hidden ? styles.hidden : styles["dialog-wrapper"]}
    >
        ...
        >
          ...
          >
            ...
            <input
              ...
              placeholder="Дата сдачи (дд.мм.гггг)"
            ...
            {uploadTodo ? (
              <div style={{ width: "100%", height: "100%" }}>
                <textarea
                  ...
                  onChange={onSubtasksChanged}
                  placeholder="Подзадачи (с новой строки)"
                  value={creationSubtasks}
                  className={styles.textbox}
                />
              </div>
            ) : (
              <div>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <div
                    className={styles["upload-progress-bar"]}
                    style={{ backgroundColor: "gray", position: "absolute" }}
                  ></div>
                </div>
              </div>
            )}
            ...
            onClick={() => {
              authClient.get("/channels/").then((response) => {
                ...
                if (uploadTodo) {
                  ...
                  authClient
                    .post("/tasks/document", formData, {
                      onUploadProgress: (progressEvent: AxiosProgressEvent) => {
                        setUploadProgress(
                          `${Math.trunc((100 * progressEvent.loaded) / progressEvent.total!)}%`,
                        );
                      },
                    })
                    .then((response) => {
