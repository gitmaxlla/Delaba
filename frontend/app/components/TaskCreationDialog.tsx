            ...
            {uploadTodo ?
              ...
              <div>
                ...
                <div style={{ position: "relative" }}>
                  <div className={styles["upload-progress-bar"]} style={{ position: "absolute", zIndex: 100, width: uploadProgress }}></div>
                  <div className={styles["upload-progress-bar"]} style={{ backgroundColor: "gray", position: "absolute" }}></div>
                </div>
              </div>
            }
          </div>
          ...
          <input onClick={() => {
            authClient.get('/channels/').then((response) => {
              ...
              if (uploadTodo) {
                ...
                authClient.post("/tasks/document", formData,
                  {
                    onUploadProgress: (progressEvent: AxiosProgressEvent) => {
                      setUploadProgress(`${Math.trunc((100 * progressEvent.loaded) / progressEvent.total!)}%`)
                    }
                  }
                ).then((response) => {
                  const newTaskId = response.data
