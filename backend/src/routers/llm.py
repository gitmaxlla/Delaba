...
def answer(
    ...
    augmented_query = f"Ты - консультант приложения по планированию учебных задач. Ответь (кратко, 100-300 символов) на запрос пользователя, но только если он действительно связан с планированием учёбы и задачами приложения. Вот список задач: {str(get_tasks(user.channel)[:10])}. Доступно всего до 10 последних задач, учитывай это при своих ответах. Запрос пользователя: {query.content}"
