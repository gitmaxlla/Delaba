...
from src.services.news import add_news
from src.services.tasks import add_todo_task
...
def mock_data():
    ...
    sections = [fake.word() for _ in range(5)]
    users = get_all_users()

    for channel in channels:
        create_channel(ChannelCreate(channel=channel))
    ...
    for _ in range(TODOS):
        add_todo_task(
            TodoTaskCreate(
                ...
                channel=random.choice(channels),
                deadline=fake.date_time_this_year(after_now=True),
                subtasks=fake.words(3),
            )
        )
